...
public class BeatBox {
    ...
    String[] instrumentNames = {"Bass Drum", "Closed Hi-Hat", "Open Hi-Hat", "Acoustic Snare", "Crash Cymbal", "Hand Clap","High Tom", "Hi Bongo", "Maracus", "Whistle", "Low Conga",  "Cowbell", "Vibraslap", "Low-mid Tom", "High Agogo", "Open Hi Conga"};
    ...
    private void buildTrackAndStart() {
        ...
        for (int i= 0; i < 16; i++) {
            ...
            for (int j = 0; j < 16; j++) {
                JCheckBox jo = checkboxList.get(j + 16 * i);
                if (jo.isSelected()) {
                    trackList[j] = key;
                } else {
                    trackList[j] = 0;
    ...
    private void makeTracks(int[] list) {
        for (int i = 0; i < 16; i++) {
            int key = list[i];
            if (key != 0) {
                track.add(makeEvent(NOTE_ON, 9, key, 100, 1));
